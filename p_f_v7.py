...
#    if the suit of card1 isn't 0 or 1, change its suit to 1
    ...
    if card2[0] == card3[0] and card2[1] > card3[1]:
        ...
        card2 = card3
        card3 = temp_card
    # if the rank of card3 = 12 and its suit = 3 or 4, redeal
#    if card3[0] == 12:
#        if card3[1] == 2 or card3[1] == 3:
#            return dealHands()
    # redeal
#    if card2[0] == 12 and card2[1] == 0:
#        if card3[0] == 12 and card3[1] == 1:
#            return dealHands()
    x_hand.append(card0)
    x_hand.append(card1)
    y_hand.append(card2)
...
def createVisualization(x_hand, y_hand, x_theta, y_theta):
    ...
    for i in range(13):
        for j in range(13):
            x_phi_aggressive, x_phi_passive = makeFeatures([[i, 0], [j, i >= j]], True), makeFeatures([[i, 0], [j, i >= j]], False)
            x_EV_push, x_EV_fold = computeSum(x_phi_aggressive, x_theta), computeSum(x_phi_passive, x_theta)
            
            y_phi_aggressive, y_phi_passive = makeFeatures([[i, 0], [j, i >= j]], True), makeFeatures([[i, 0], [j, i >= j]], False)
...
#        x_push, y_call = True, True
        ...
        if n % 100000 == 0:
            print("\n")
#            print(x_equity, y_equity, 1 - (x_equity + y_equity))
#            print(x_push, y_call)
